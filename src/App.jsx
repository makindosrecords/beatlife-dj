...
const App = () => {
  ...
  useEffect(() => {
    // 4. Fetch Instagram Feed via Vercel Serverless Function
             ...
             <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
               {instagramPosts.map((post) => (
                 <a 
                   ...
                   target="_blank" 
                   rel="noopener noreferrer"
                   className="group relative aspect-[9/16] rounded-[1.5rem] overflow-hidden bg-neutral-900 border border-white/5 hover:border-cyan-500/50 transition-all duration-500 shadow-lg block"
                   onMouseEnter={(e) => {
                 ...
                 >
                   <img 
                     ...
                     className={`relative z-10 w-full h-full object-cover transition-all duration-700 ${post.media_type === 'VIDEO' ? 'opacity-80 group-hover:opacity-0' : 'opacity-80 group-hover:opacity-100 group-hover:scale-110'}`}
                     loading="lazy"
                   />
                   ...
                   <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end items-center p-4 text-center z-20 pb-6">
                      ...
                      <p className="text-white text-[10px] md:text-xs font-medium line-clamp-3 transform translate-y-4 group-hover:translate-y-0 transition-all duration-300 delay-75 uppercase italic">
                        {post.caption ? post.caption.slice(0, 75) + (post.caption.length > 75 ? '...' : '') : 'View on Instagram'}
                      </p>
                   </div>
                 </a>
               ))}
